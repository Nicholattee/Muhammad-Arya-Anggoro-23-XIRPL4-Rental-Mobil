...
//erwbdoidrfdibinriosibsijfoisjdfonsdvoesingosiaonvassvanwofoubaaubsouwovunoawousajvo)
    ...
    # jika update data dan gambar
        ...
        if (move_uploaded_file($_FILES["foto"]["tmp_name"], $folder)) {
            if (empty($_POST["password"])) {
                ...
                kontak='$kontak', username='$username', foto='$foto' where id_karyawan='$id_karyawan'";
            } else {
                $password = sha1($_POST["password"]);
                $sql = "update karyawan set id_karyawan='$id_karyawan', nama_karyawan='$nama_karyawan',
